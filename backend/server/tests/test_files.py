...
class APITests(TestCase):
    ...
    def test_load_file(self):
        ...
        self.assertEqual(response_2.json(), {
            ...
            'last_download_date': None,
            'comment': '',
            'path': response_2.json()['path'],
            'unique_id': response_2.json()['unique_id']
        })
        self.assertEqual(response_3.json(), {
            'id': response_3.json()['id'],
            'file_name': 'testfile2_2.jpg',
            'user': self.testuser.id,
            'file': f'/storage/{self.testuser.user_folder}/testfile2_2.jpg',
            'size': 674769,
            'upload_date': datetime.today().strftime('%Y-%m-%d'),
            'last_download_date': None,
            'comment': '',
            'path': response_3.json()['path'],
            'unique_id': response_3.json()['unique_id']
        })
